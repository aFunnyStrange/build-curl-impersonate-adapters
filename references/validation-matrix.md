...
| Context | every declared navigation/fetch/subresource/preflight/WebSocket scenario has observed or bounded evidence |
| Adapter unit | one-directory activation, alias/context mapping, pass-through, conflict, no default, import-order rejection |
| Pooling | different native targets cannot share an established pool; same transport can safely carry supported contexts |
| curl_cffi | sync/async scenario coverage with target-isolated Sessions/pools, official, plain, cleanup |
| scrapy_cffi | request-context mapping plus a real profile-safe transport request for every required scenario |
...
For fingerprint validation, require profile probe success, expected browser-family/version and request-context
header fields, declared TLS capabilities, negotiated protocol, capture-derived H2/H3 parameters, and agreement
between semantic alias, native target, context, and pool identity. Record variable JA3/JA4 values; assert only
invariants justified by multiple captures or current official baselines.
